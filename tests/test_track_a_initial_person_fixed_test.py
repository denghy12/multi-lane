...
from multi_lane.track_a.fuse_initial_person_fixed_test_scores import (
    fuse_initial_person_fixed_test_runs,
)
...
class InitialPersonFixedTest(unittest.TestCase):
    @staticmethod
    def _write_test_run(root: Path, input_mode: str, offset: float) -> None:
        ...
        for task_id, seen_classes in enumerate(np.cumsum(TASK_SIZES)):
            ...
            rows.append(compute_metrics(task_id, torch.sigmoid(logits), targets, 0.5))
        summary = {
            "status": "complete",
            "config": config,
        ...
        (root / "seed_summary.json").write_text(json.dumps(summary), encoding="utf-8")
    ...
    @staticmethod
    def _set_initial_transform(run: Path, transform: str = "legacy_crop") -> None:
        for filename in ("config.json", "seed_summary.json"):
            path = run / filename
            payload = json.loads(path.read_text(encoding="utf-8"))
            config = payload if filename == "config.json" else payload["config"]
            config.update(
                {
                    "person_transform_mode": transform,
                    "person_crop_margin": 0.15,
                    "train_crop_scale": [0.70, 1.0],
                    "person_color_jitter_strength": 0.0,
                    "person_color_jitter_probability": 0.0,
                    "git": {"commit": "person456", "dirty": False},
                }
            )
            path.write_text(json.dumps(payload), encoding="utf-8")

    def test_applies_one_fixed_rule_to_initial_person_transform(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            full, person = root / "full", root / "person"
            selection = root / "validation.json"
            self._write_test_run(full, "full", 0.0)
            self._write_test_run(person, "person_crop", -0.2)
            self._set_initial_transform(person)
            ...
            self.assertEqual(result["evaluated_fusion_rule_count"], 1)
            self.assertFalse(result["search_performed_on_test"])
            self.assertEqual(result["locked_rule"]["person_weight"], 0.20)
            self.assertEqual(
                result["sources"]["person_transform"]["mode"], "legacy_crop"
            )
            self.assertNotIn("candidates", result)

    def test_rejects_letterbox_person_source(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            full, person = root / "full", root / "person"
            selection = root / "validation.json"
            self._write_test_run(full, "full", 0.0)
            self._write_test_run(person, "person_crop", -0.2)
            self._set_initial_transform(person, "letterbox")
            self._write_selection(selection)
            with self.assertRaisesRegex(ValueError, "initial legacy crop"):
                fuse_initial_person_fixed_test_runs(
                    full,
                    person,
                    selection,
