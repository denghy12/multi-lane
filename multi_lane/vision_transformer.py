...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        pret_attention = True if args.method in ['prompts'] else False
        prompts = [False] * 12
        prompts = [True] * args.num_prompt_layers + [False] * (12 - args.num_prompt_layers)
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                pret_attention=pret_attention, prompts=prompts[i], id=i)
    ...
    def forward_features(self, x):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            # get selectors for current task if training, all tasks so far otherwise
            if hasattr(self, 'selectors'):
                ...
                task_tokens = torch.zeros((task_num, self.num_selectors, self.embed_dim))
                task_tokens = task_tokens.to(x.device)
            
            # add batch dimension to selectors
            task_tokens = task_tokens.unsqueeze(1).expand(-1, x.size(0), -1, -1)

            # expand class token to task and batch dimension and add to task tokens
    ...
    def forward_head(self, x, task_tokens, eval: bool=False):
        assert self.global_pool != 'avg'
        x = x[:, 0]

        sim = torch.tensor([1.])
        tasks = torch.tensor([0] * x.size(0))
        if self.head_mode == 'task' or not eval:
            if hasattr(self, 'task_identifier'):
                tasks, sim = self.task_identifier(x.detach())

            features = task_tokens.permute(1, 0, 2, 3).clone()
            
            task_tokens = task_tokens[:, :, 0].permute(1, 0, 2)
            task_tokens = task_tokens[torch.arange(task_tokens.size(0)), tasks]
            if self.normalize == 'pre-head':
                task_tokens = F.normalize(task_tokens, dim=-1)

            logits = self.head(task_tokens)

            return logits, features, x, sim, tasks
        
        elif self.head_mode == 'concat':
            T, B, P, D = task_tokens.size()
            task_tokens = task_tokens[:, :, 0].permute(1, 0, 2)
            ...
            return logits, task_tokens, x, sim, tasks

    def forward(self, x, eval: bool=False):
        feats, task_feats = self.forward_features(x)
        logits, feats, frozen_feats, sim, tasks = self.forward_head(feats, task_feats, eval=eval)
    ...
    def get_ortho_loss(self):
        ...
        for block in self.blocks:
            if block.prompts:
                ortho_loss += block.get_ortho_loss()
        return ortho_loss
