"""OpenAI CLIP model definition used by the frozen visual checkpoint.

Derived from https://github.com/openai/CLIP under the MIT license.  Kept local
so the formal reproduction does not depend on another worktree or a network
download at runtime.
"""
...
class ResidualAttentionBlock_visual(nn.Module):
    ...
    def forward(self, x: torch.Tensor, visual_prompts=None):
        if visual_prompts is not None:
            ...
            attn_out = self.attention(self.ln_1(q),self.ln_1(k),self.ln_1(v))
            x = x + attn_out[visual_prompts.size(0):]
        else:
            q = x
            k = x
            v = x
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        scale = width ** -0.5
        self.class_embedding = nn.Parameter(scale * torch.randn(width))
        self.positional_embedding = nn.Parameter(scale * torch.randn((input_resolution // patch_size) ** 2 + 1, width)) # shape = [grid ** 2 + 1, width]
        self.ln_pre = LayerNorm(width)
    ...
    for k, v in old_state_dict.items():
        if k in state_dict.keys():
            if 'visual' in k and ('v_proj' in k or 'c_proj' in k):
                ...
                w_spacial = state_dict[k][1:].permute(1, 0).reshape(1, -1, 14, 14)
