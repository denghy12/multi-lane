...
case "${METHOD}" in
  Shared-live) INIT=zero_output; COMPONENTS=all; FREEZE_CENTER=0; TASK_LOCAL=0 ;;
  Frozen-center) INIT=zero_output; COMPONENTS=router; FREEZE_CENTER=1; TASK_LOCAL=0 ;;
  Task-local-delta) INIT=zero_output; COMPONENTS=router; FREEZE_CENTER=1; TASK_LOCAL=1 ;;
...
echo "ParaX center stability method=${METHOD} seed=0 gpu=${GPU} tasks=3 epochs=30 batch=64 main_lr=0.0125 parax_lr=0.0004 rank=32 experts=3 router_hidden=16 layer=10 init=${INIT} fixed_three_view aux=0.1 zero_output fixed_center=${FREEZE_CENTER} task_local_gate=${TASK_LOCAL} reporting=val checkpoint=off test=forbidden AMP=on TF32=on"
ARGS=(
  ...
  --parax-layer-indices 10 --parax-router-hidden 16 --parax-residual-scale 0.1
  --parax-initialization "${INIT}" --parax-trainable-components "${COMPONENTS}"
  --parax-output-scale-mode fixed --parax-residual-ratio-cap 0
  --parax-residual-penalty-weight 0
...
if [[ "${TASK_LOCAL}" == 1 ]]; then ARGS+=(--parax-task-local-gate); fi
if [[ "${FREEZE_CENTER}" == 1 ]]; then
  ARGS+=(--parax-freeze-center-after-task0)
fi
