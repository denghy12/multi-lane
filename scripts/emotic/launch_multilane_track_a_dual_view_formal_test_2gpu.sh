...
VALIDATION_FUSION_SUMMARY="${VALIDATION_FUSION_SUMMARY:-/mnt/haoyuan/workspace/multi-lane-main-dual-view/output/emotic_track_a_dual_view_val/image_token_layer1_full_person_letterbox_val_seed0_20260903_134720/fusion_summary.json}"
LOCKED_VALIDATION_SHA256="a03ff4da93e18bcfde4b4e8898639608e341dfff335d6f66fc355ee4df96b90f"

[[ "${#GPU_LIST[@]}" -eq 2 ]] || { echo "Formal dual-view test requires two GPUs" >&2; exit 2; }
[[ "${GPU_LIST[0]}" != "${GPU_LIST[1]}" ]] || { echo "GPUs must be distinct" >&2; exit 2; }
[[ -z "$(git status --porcelain)" ]] || { echo "Formal test launch requires a clean Git worktree" >&2; exit 2; }
...
[[ -f "${VALIDATION_FUSION_SUMMARY}" ]] || { echo "Missing locked validation selection: ${VALIDATION_FUSION_SUMMARY}" >&2; exit 2; }
actual_validation_sha256="$(sha256sum "${VALIDATION_FUSION_SUMMARY}" | awk '{print $1}')"
[[ "${actual_validation_sha256}" == "${LOCKED_VALIDATION_SHA256}" ]] || {
  echo "Validation fusion summary hash differs from the locked artifact" >&2
  exit 2
}

"${PYTHON}" - "${VALIDATION_FUSION_SUMMARY}" <<'PY'
import json
import math
import sys
...
if summary.get("selection_split") != "val":
    raise SystemExit("Fusion rule was not selected on validation")
if winner.get("mode") != "probability" or not math.isclose(
    float(winner.get("alpha", -1)), 0.20, abs_tol=1e-12
):
    raise SystemExit("Validation winner is not locked probability alpha=0.20")
if not summary.get("decision", {}).get("advance_to_formal_test"):
    raise SystemExit("Validation did not approve formal test")
...
printf 'view\tgpu\trun_id\tinput\tmargin\ttransform\tjitter\treporting\tfusion\n' > "${MANIFEST}"
printf 'full\t%s\t%s\tfull\t0\tlegacy_crop\t0@0\ttest\tprobability_weight_0.80\n' "${GPU_LIST[0]}" "${FULL_RUN_ID}" >> "${MANIFEST}"
printf 'person\t%s\t%s\tperson_crop\t0.15\tletterbox\t0.10@0.20\ttest\tprobability_weight_0.20\n' "${GPU_LIST[1]}" "${PERSON_RUN_ID}" >> "${MANIFEST}"
printf '%s\n' "${VALIDATION_FUSION_SUMMARY}" > "${CONTROL_DIR}/validation_selection_path.txt"
...
if (( failed != 0 )); then
  echo "At least one formal test run failed; completed outputs are preserved" >&2
  exit 1
fi

"${PYTHON}" -m multi_lane.track_a.fuse_fixed_test_scores \
  --full-run "${OUTPUT_BASE}/${FULL_RUN_ID}" \
  --person-run "${OUTPUT_BASE}/${PERSON_RUN_ID}" \
  --validation-fusion-summary "${VALIDATION_FUSION_SUMMARY}" \
  --output "${CONTROL_DIR}/fixed_test_fusion_summary.json"
printf 'complete\n' > "${CONTROL_DIR}/batch_status.txt"
echo "MULTI_LANE_DUAL_VIEW_FORMAL_TEST_COMPLETE batch_id=${BATCH_ID} output=${OUTPUT_BASE} fusion=${CONTROL_DIR}/fixed_test_fusion_summary.json"
