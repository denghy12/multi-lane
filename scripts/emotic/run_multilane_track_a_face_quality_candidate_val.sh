...
LOG_DIR="${LOG_DIR:-${ROOT}/logs/emotic_track_a_face_quality}"
RUN_ROOT="${OUTPUT_BASE}/${RUN_ID}"

case "${VARIANT}" in
  reliable_m15) margin=0.15; jitter_strength=0; jitter_probability=0 ;;
  reliable_m05) margin=0.05; jitter_strength=0; jitter_probability=0 ;;
  reliable_m05_jitter) margin=0.05; jitter_strength=0.1; jitter_probability=0.5 ;;
  *) echo "Unknown Face quality variant: ${VARIANT}" >&2; exit 2 ;;
esac
...
echo "Face quality validation: variant=${VARIANT} seed=0 gpu=${GPU} dataset=EMOTIC train_filter=valid_nonambiguous_shortside24_score0.6 margin=${margin} letterbox224 jitter=${jitter_strength}@${jitter_probability} tasks=8 epochs=30 batch=64 cosine_min0_nowarmup layer1_b32_adapterLR4e-4_scale0.1_relu_independent mainBCE_adapterASL9.8_0_0.05 AMP_TF32 val_scores no_checkpoint test_forbidden"
CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON}" -m multi_lane.track_a.runner \
  ...
  --face-crop-margin "${margin}" --face-min-training-short-side 24 \
  --face-min-training-score 0.6 --face-color-jitter-strength "${jitter_strength}" \
  --face-color-jitter-probability "${jitter_probability}" \
...
echo "FACE_QUALITY_CANDIDATE_COMPLETE variant=${VARIANT} run=${RUN_ROOT}"
