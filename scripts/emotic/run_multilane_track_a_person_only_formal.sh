...
LOG_DIR="${LOG_DIR:-${ROOT}/logs/emotic_track_a_person_only_formal}"
PERSON_CROP_MARGIN="${PERSON_CROP_MARGIN:-0.15}"
PERSON_TRAIN_CROP_MIN="${PERSON_TRAIN_CROP_MIN:-0.70}"
...
echo "Person-only formal: run_id=${RUN_ID} seed=0 gpu=${GPU} dataset=EMOTIC input=person_crop bbox_margin=${PERSON_CROP_MARGIN} train_crop_scale=${PERSON_TRAIN_CROP_MIN},1.0 normalization=clip tasks=8 epochs=30 batch=64 scheduler=cosine eta_min=0 warmup=0 layer=1 bottleneck=32 main_lr=0.0125 adapter_lr=0.0004 adapter_weight_decay=0 scale=0.1 activation=relu init=independent main_loss=bce adapter_loss=asl9.8/0/0.05 amp=on tf32=on reporting=test checkpoints=disabled"
CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON}" -m multi_lane.track_a.runner \
  ...
  --asl-eps 1e-8 \
  --no-save-checkpoints \
  --input-mode person_crop \
  --person-crop-margin "${PERSON_CROP_MARGIN}" \
  --input-normalization clip \
  --train-crop-scale "${PERSON_TRAIN_CROP_MIN}" 1.0 \
...
echo "MULTI_LANE_PERSON_ONLY_FORMAL_COMPLETE run_root=${RUN_ROOT} log=${LOG_PATH}"
