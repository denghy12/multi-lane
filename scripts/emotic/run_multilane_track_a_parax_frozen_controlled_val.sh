...
case "${METHOD}" in
  B0-paired) PARAX_MODE=disabled; SCALE=0.1; PENALTY=0 ;;
  Frozen-center-small) PARAX_MODE=image; SCALE=0.001; PENALTY=0 ;;
  Frozen-center-penalty) PARAX_MODE=image; SCALE=0.1; PENALTY=1.0 ;;
...
echo "ParaX controlled Frozen-center method=${METHOD} seed=0 gpu=${GPU} tasks=3 epochs=30 batch=64 main_lr=0.0125 parax_lr=0.0004 rank=32 experts=3 layer=10 scale=${SCALE} penalty=${PENALTY} fixed_three_view validation_only checkpoint=off test=forbidden AMP=on TF32=on"
ARGS=(
  ...
  --parax-router-hidden 16
  --parax-initialization zero_output --parax-trainable-components router
  --parax-output-scale-mode fixed --parax-residual-scale "${SCALE}"
  --parax-residual-ratio-cap 0 --parax-residual-penalty-weight "${PENALTY}"
  --parax-freeze-center-after-task0
