...
[[ -z "$(git status --porcelain)" ]] || { echo "Validation run requires a clean Git worktree" >&2; exit 2; }
mkdir -p "${LOG_DIR}" "${OUTPUT_BASE}"

echo "Dual-view validation: run_id=${RUN_ID} view=${VIEW} seed=${SEED} gpu=${GPU} dataset=EMOTIC input=${input_mode} bbox_margin=${person_margin} person_transform=${person_transform} person_jitter=${person_jitter_strength}@${person_jitter_probability} train_crop_scale=${crop_min},1.0 normalization=clip tasks=8 epochs=30 batch=64 scheduler=cosine eta_min=0 warmup=0 layer=1 bottleneck=32 main_lr=0.0125 adapter_lr=0.0004 adapter_weight_decay=0 scale=0.1 activation=relu init=independent main_loss=bce adapter_loss=asl9.8/0/0.05 amp=on tf32=on reporting=val score_dump=on checkpoints=disabled"
...
echo "DUAL_VIEW_VALIDATION_RUN_COMPLETE view=${VIEW} run_root=${RUN_ROOT} log=${LOG_PATH}"
