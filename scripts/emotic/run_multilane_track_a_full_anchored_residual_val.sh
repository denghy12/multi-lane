...
METHOD="${METHOD:?METHOD is required (A0, A1, or A2)}"
...
LOG_DIR="${LOG_DIR:-${ROOT}/logs/emotic_track_a_full_anchored_residual}"

case "${METHOD}" in
  A0) fusion_mode=disabled; needs_face=0 ;;
  A1) fusion_mode=residual_full_person; needs_face=0 ;;
  A2) fusion_mode=residual_three_view; needs_face=1 ;;
  *) echo "METHOD must be A0, A1, or A2" >&2; exit 2 ;;
esac
...
CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON}" -m multi_lane.track_a.runner \
  ...
  --view-residual-scale 0.1 \
  --view-auxiliary-loss-weight 0 \
  "${face_args[@]}" \
...
echo "FULL_ANCHORED_RESIDUAL_VALIDATION_COMPLETE method=${METHOD} run=${RUN_ROOT}"
