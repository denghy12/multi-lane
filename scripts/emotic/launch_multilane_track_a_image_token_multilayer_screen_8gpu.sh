...
[[ "${#GPU_LIST[@]}" -eq 8 ]] || { echo "Multilayer screen requires exactly 8 GPUs" >&2; exit 2; }
for gpu in "${GPU_LIST[@]}"; do
  [[ "${gpu}" =~ ^[0-9]+$ ]] || { echo "GPU identifiers must be nonnegative integers" >&2; exit 2; }
done
[[ "$(printf '%s\n' "${GPU_LIST[@]}" | sort -u | wc -l | tr -d ' ')" -eq 8 ]] || {
  echo "Multilayer screen requires 8 distinct GPUs" >&2
  exit 2
}
[[ "${GPU_READY_CHECKS}" =~ ^[1-9][0-9]*$ ]] || { echo "GPU_READY_CHECKS must be positive" >&2; exit 2; }
[[ -z "$(git status --porcelain)" ]] || { echo "Multilayer screen requires a clean Git worktree" >&2; exit 2; }
...
BATCH_ID="${BATCH_ID:-image_token_multilayer_screen_seed0_$(date +%Y%m%d_%H%M%S)}"
OUTPUT_BASE="${OUTPUT_BASE:-${ROOT}/output/emotic_image_token_tuning/multilayer_screen/${BATCH_ID}}"
LOG_DIR="${LOG_DIR:-${ROOT}/logs/emotic_image_token_tuning/multilayer_screen}"
...
append_pair() {
  local structure="$1"
  local layers="$2"
  labels+=("${structure}_bce" "${structure}_asl")
  structures+=("${structure}" "${structure}")
  adapter_modes+=(image_token image_token)
  routings+=(joint_bce adapter_asl)
  layer_specs+=("${layers}" "${layers}")
}
append_pair single3 "3"
append_pair single11 "11"
append_pair pair2_3 "2 3"
append_pair pair3_7 "3 7"
append_pair pair3_11 "3 11"
append_pair late8_9 "8 9"
append_pair late_blocks8_12 "7 8 9 10 11"

printf 'index\tlabel\tstructure\tadapter_mode\tloss_routing\tzero_based_layers\tbottleneck\tadapter_lr\tresidual_scale\tactivation\ttask_init\tprecision\tgpu\n' > "${MANIFEST}"
for index in "${!labels[@]}"; do
  gpu="${GPU_LIST[$((index % ${#GPU_LIST[@]}))]}"
  printf '%s\t%s\t%s\t%s\t%s\t%s\t32\t0.0004\t0.1\trelu\tindependent\tfp32\t%s\n' \
    "${index}" "${labels[$index]}" "${structures[$index]}" \
    "${adapter_modes[$index]}" "${routings[$index]}" \
    "${layer_specs[$index]}" "${gpu}" >> "${MANIFEST}"
done
...
wait_for_any_gpu_ready() {
  ...
  for index in "${!GPU_LIST[@]}"; do consecutive[$index]=0; done
  while true; do
    for index in "${!GPU_LIST[@]}"; do
      ...
      if (( free_mib >= GPU_MIN_FREE_MIB && utilization <= GPU_MAX_UTIL_PERCENT )); then
        ...
        echo "GPU ${gpu} ready ${consecutive[$index]}/${GPU_READY_CHECKS} for global multilayer smoke: free_mib=${free_mib} utilization=${utilization}%" >&2
      else
        consecutive[$index]=0
        echo "GPU ${gpu} waiting for global multilayer smoke: free_mib=${free_mib} utilization=${utilization}%" >&2
      fi
...
CUDA_VISIBLE_DEVICES="${SMOKE_GPU}" "${PYTHON}" -m multi_lane.track_a.smoke \
  ...
  --adapter-layer-indices 3 11 \
  --adapter-task-init independent \
  --loss-routing adapter_asl \
  --no-amp
echo "IMAGE_TOKEN_MULTILAYER_SCREEN_FP32_SMOKE_COMPLETE gpu=${SMOKE_GPU} layers=3,11"
...
run_gpu_lane() {
  ...
  for ((job_index=lane_index; job_index<${#labels[@]}; job_index+=${#GPU_LIST[@]})); do
    ...
    echo "Starting ${label} layers=${layer_specs[$job_index]} on physical GPU ${gpu} in FP32"
    if SEED=0 GPU="${gpu}" ADAPTER_MODE="${adapter_modes[$job_index]}" \
      LOSS_ROUTING="${routings[$job_index]}" \
      ADAPTER_LAYERS="${layer_specs[$job_index]}" \
...
if [[ "${failed}" != "0" ]]; then
  echo "At least one multilayer-screen run failed; completed runs were preserved" >&2
  exit 1
fi
...
"${PYTHON}" -m multi_lane.track_a.summarize_image_token_multilayer_search \
  "${summary_arguments[@]}" \
  --output "${OUTPUT_BASE}/multilayer_search_summary.json"
echo "MULTI_LANE_IMAGE_TOKEN_MULTILAYER_SCREEN_COMPLETE ${OUTPUT_BASE}"
