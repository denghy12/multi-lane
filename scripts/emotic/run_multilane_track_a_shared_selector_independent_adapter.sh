...
LOG_DIR="${LOG_DIR:-${ROOT}/logs/emotic_track_a_shared_selector_independent_adapter}"

case "${METHOD}" in
  A0-shared-b32)
    shared_bottleneck=32
    view_bottleneck=0
    view_mode=shared
    expected_parameters=49952
    ;;
  A-cap-shared-b97)
    shared_bottleneck=97
    view_bottleneck=0
    view_mode=shared
    expected_parameters=149857
    ;;
  A-view-independent-b32)
    shared_bottleneck=32
    view_bottleneck=32
    view_mode=independent
    expected_parameters=149856
    ;;
...
echo "Shared-Selector independent-Adapter control: method=${METHOD} shared_b${shared_bottleneck} view_mode=${view_mode} view_b${view_bottleneck} expected_parameters=${expected_parameters} seed0 EMOTIC tasks0-2 30epochs/task batch64 mainLR0.0125 cosine_min0_nowarmup fixed_R1_feature_fusion aux0.1 joint_gradient mainBCE adapterASL9.8/0/0.05 layer1 adapterLR4e-4 scale0.1 relu residual_ratio_diagnostic AMP_TF32 val_only no_checkpoint test_forbidden"
...
echo "SHARED_SELECTOR_INDEPENDENT_ADAPTER_COMPLETE method=${METHOD} run=${RUN_ROOT}"
