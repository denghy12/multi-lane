...
echo "Initial Person score test: run_id=${RUN_ID} seed=0 gpu=${GPU} dataset=EMOTIC input=person_crop bbox_margin=0.15 person_transform=legacy_crop person_jitter=0@0 train_crop_scale=0.70,1.0 normalization=clip tasks=8 epochs=30 batch=64 scheduler=cosine eta_min=0 warmup=0 layer=1 bottleneck=32 main_lr=0.0125 adapter_lr=0.0004 scale=0.1 activation=relu init=independent main_loss=bce adapter_loss=asl9.8/0/0.05 amp=on tf32=on reporting=test scores=on checkpoints=off"
CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON}" -m multi_lane.track_a.runner \
  ...
  --person-transform-mode legacy_crop \
...
echo "INITIAL_PERSON_SCORE_TEST_COMPLETE run_root=${RUN_ROOT} log=${LOG_PATH}"
