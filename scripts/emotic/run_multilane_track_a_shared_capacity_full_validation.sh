...
LOG_DIR="${LOG_DIR:-${ROOT}/logs/emotic_track_a_shared_capacity_full}"
case "${METHOD}" in
  A0-shared-b32) shared_bottleneck=32; expected_parameters=49952 ;;
  A-cap-shared-b97) shared_bottleneck=97; expected_parameters=149857 ;;
...
echo "Shared capacity full validation: method=${METHOD} shared_b${shared_bottleneck} expected_parameters=${expected_parameters} seed0 tasks0-7 validation-only test-forbidden"
CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON}" -m multi_lane.track_a.runner \
  ...
  --adapter-layer-indices 1 --adapter-residual-scale 0.1 --adapter-residual-gate-mode fixed \
  ...
  2>&1 | tee "${LOG_PATH}"
echo "SHARED_CAPACITY_FULL_VALIDATION_COMPLETE method=${METHOD} run=${RUN_ROOT}"
