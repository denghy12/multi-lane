...
case "${METHOD}" in
  ...
  V-post-residual) FUSION=residual_three_view ;;
  *) echo "Unknown METHOD=${METHOD}" >&2; exit 2 ;;
esac
RUN_ROOT="${OUTPUT_ROOT}/${RUN_ID}"
[[ ! -e "${RUN_ROOT}" ]] || { echo "Output already exists: ${RUN_ROOT}" >&2; exit 2; }
mkdir -p "$(dirname "${LOG_PATH}")"
echo "Post-feature calibration control method=${METHOD} seed=0 gpu=${GPU} tasks=3 epochs=30 batch=64 main_lr=0.0125 fusion_lr=0.0004 fusion=${FUSION} residual_scale=0.1 parax=disabled adapter=disabled validation_only checkpoint=off test=forbidden AMP=on TF32=on"
CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON}" -m multi_lane.track_a.runner \
  ...
  --view-residual-scale 0.1 --view-auxiliary-loss-weight 0 --view-gradient-routing joint \
