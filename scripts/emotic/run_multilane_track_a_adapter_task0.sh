...
RUN_ID="${RUN_ID:-task_lane_adapter_task0_seed${SEED}_b${BOTTLENECK}_lr${ADAPTER_LR}_$(date +%Y%m%d_%H%M%S)}"
OUTPUT_BASE="${OUTPUT_BASE:-/mnt/haoyuan/workspace/emotic_benchmark_runs/multi_lane_task_lane_adapter_screen_v0.1}"
...
LOG_DIR="${ROOT}/logs/emotic_track_a_adapter"
...
echo "Task-0 validation screen: seed=${SEED} gpu=${GPU} bottleneck=${BOTTLENECK} adapter_lr=${ADAPTER_LR}"
