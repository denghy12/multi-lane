...
torchrun --nproc_per_node=1 \
  ...
  --nnodes=1 \
  main.py voc \
  --name voc_ddp_official_semantic_tau2_g07_20ep \
  --notes "official DDP semantic recipe, optimized PCD tau2 gamma0.7, 20ep full run" \
  --data_path ./datasets/VOC2007 \
  --output_dir ./output/voc_ddp_official_semantic_tau2_g07_20ep \
  --num_tasks 5 \
  --base_classes 0 \
  --epochs 20 \
  --batch_size 8 \
  ...
  --store_model \
  "$@" \
  2>&1 | tee ./logs/voc_ddp_official_semantic_tau2_g07_20ep.log
